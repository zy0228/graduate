      ...
      <div class="checkout-order">
        ...
        <div class="check-step">
          <ul>
            ...
            <li class="cur"><span>浏览订单</span></li>
            <li><span>支付</span></li>
        ...
        <div class="item-list-wrap confirm-item-list-wrap">
          <div class="cart-item order-item">
            ...
            <ul class="cart-item-list">
              <li v-for="(item, index) in orderList" :key="index" v-if="item.checked == 1">
        ...
        <div class="order-foot-wrap">
          ...
          <div class="next-btn-wrap">
            <button class="btn btn--m btn--red" @click="orderPay">立即支付</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
      ...
      axios.get('/users/cart').then(response => {
        ...
        if(res.status == 0) {
          ...
          this.orderList.forEach(item=> {
            if(item.checked == 1) {
              this.subTotal += item.productNum * item.prodcutPrice 
            }
          })
    ...
    orderPay() {
      let address = this.$route.query.addressId
      axios.post('/users/pay', {addressId: address,orderTotal: this.orderTotal}).then(response => {
        let res = response.data
        if(res.status == 0) {
          this.$router.push({path: '/orderSuc?userId='+res.result.orderId})
        }
      })
    }
  }
