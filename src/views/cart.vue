<template>
  <div>
              ...
              <li>
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" v-bind:class="{'check':item.checked=='1'}" @click="edit('checked',item)">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="edit('produce',item)">-</a>
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{item.productNum * item.prodcutPrice}}</div>
                </div>
          ...
          <div class="cart-foot-inner">
            ...
            <div class="cart-foot-r">
              <div class="item-total">
                总额: <span class="total-price">{{total}}</span>
              </div>
              <div class="btn-wrap">
                <a class="btn btn--red" @click="put">提交</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <hxFooter></hxFooter>
    <cartModal :mdshow="modalConfirm" @close="closeModal">
      <p slot="message">
        你确认要移除此商品吗？
      </p>
      <div slot="btnGroup">
        <a class="btn btn--m" @click="delCart">确定</a>
        <a class="btn btn--m" @click="modalConfirm = false">取消</a>
      </div>
    </cartModal>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    total() {
      ...
      this.cartList.forEach(item => {
        if(item.checked == 1) {
          totalprice += parseFloat(item.prodcutPrice) * parseInt(item.productNum)
        }
      })
      return totalprice
    }
  ...
  methods: {
    ...
    delCart() {
      axios.post('/users/del', {productId: this.delItem}).then(response => {
        let res = response.data
        if(res.status == '0') {
    ...
    edit(flag, item) {
      ...
      axios.post('/users/editCart', {productId: item.productId, productNum: item.productNum,checked: item.checked}).then(response => {
        let res = response.data
        console.log(res)
      })
    },
