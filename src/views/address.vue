<template>
  <div>
    <hxHeader></hxHeader>
    <hxBread><span>收货地址</span></hxBread>
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li  v-for="(item, index) in adressList" :key="index" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddrId=item.addressId"> 
                  <dl>
                    <dt>{{item.userName}}</dt>
            ...
            <div class="shipping-addr-more">
              <a class="addr-more-btn up-down-btn" href="javascript:;" @click="move" :class="{'open': limit > 3}">
                更多
                <i class="i-up-down">
  ...
  <hxFooter></hxFooter>
  <modal :mdshow="addressShow" @close="closeAddress">
    <p slot="message">确认要移除该地址？</p>
    <div slot="btnGroup">
      <a class="btn btn--m" @click="del(addressId)">确定</a>
      <a class="btn btn--m" @click="addressShow = false">取消</a>
    </div>
  </modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    getaddress() {
      axios.get('/users/address').then(response => {
        ...
        if(res.status == '0') {
          ...
          this.selectedAddrId = res.result.addressId
        }
      })
    },
    move() {
      if(this.limit < this.orderList.length ) {
    ...
    setDefault(addressId) {
      axios.post('/users/setaddressDefault', {addressId: addressId}).then(response => {
        let res = response.data
        if(res.status == '0') {
    ...
    del(de) {
      axios.post('/users/delAddress', {addressId: de}).then(response => {
        let res = response.data
        if(res.status == '0'){
          this.addressShow = false
        }
...
<style scoped>
  .addr-list li:hover .addr-set-default{
    display: block;
  }
</style>
